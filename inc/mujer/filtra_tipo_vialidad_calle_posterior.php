...
<select  class="combobox"id="calle_posterior" name="calle_posterior">
    <option value=''>Seleccione vialidad posterior</option>
    <?php foreach($tipo_via as $l): 

        if($l['CVE_VIA'] == $beneficiario['calle_posterior']){
            $selected = "selected";
        }else{
            $selected = "";
        }
